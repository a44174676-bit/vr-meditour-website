(function () {
  ...
  const premiumAccessKey = 'aiSkinPremiumAccess';
  ...
  const hasStoredPremiumAccess = getStoredPremiumAccess();
  let premiumUnlocked = isPremium || hasStoredPremiumAccess;
  ...
  function getStoredPremiumAccess() {
    try {
      return sessionStorage.getItem(premiumAccessKey) === 'BUSANBLUE';
    } catch (_) {
      return false;
    }
  }

  function setStoredPremiumAccess() {
    try {
      sessionStorage.setItem(premiumAccessKey, 'BUSANBLUE');
    } catch (_) {}
  }
  ...
  async function runPremiumAnalysis() {
    ...
    setLoading(true);
    try { await analyze(pendingPremiumImage, getRecentFreeCapture()?.freeAnalysis || null); } finally { setLoading(false); }
  }
  ...
  function preparePremiumReportAfterAuth() {
    ...
    if (!recent) {
      if (premiumQuality) {
        premiumQuality.hidden = false;
        premiumQuality.innerHTML = `<h3>${esc(t('premiumAccessConfirmed'))}</h3><p>${esc(t('premiumAccessNoPhoto'))}</p>`;
      }
      return;
    }
    ...
    if (premiumQuality) {
      premiumQuality.hidden = false;
      premiumQuality.innerHTML = `<h3>${esc(t('premiumAccessConfirmed'))}</h3><div class="quality-result good"><strong>${esc(t('premiumOpened'))}</strong><br>${esc(t('premiumGeneratingFromPhoto'))}</div>`;
    }
    updatePremiumAnalyzeState();
  }

  function showPremiumUnlockedMessage() {
    statusEl.textContent = t('premiumAccessConfirmed');
  }

  async function startPremiumAnalysisFromExistingPhoto() {
    const recent = getRecentFreeCapture();
    if (!recent?.imageBase64 || !recent?.freeAnalysis) {
      if (premiumQuality) {
        premiumQuality.hidden = false;
        premiumQuality.innerHTML = `<h3>${esc(t('premiumAccessConfirmed'))}</h3><p>${esc(t('premiumAccessNoPhoto'))}</p>`;
      }
    ...
    updatePremiumAnalyzeState();
    setLoading(true);
    try { await analyze(recent.imageBase64, recent.freeAnalysis); } finally { setLoading(false); }
  }

  async function handlePremiumQrSuccess(scannedText) {
    if (!isValidPremiumQr(scannedText)) {
      showQrResult(t('qrInvalid'));
      return;
    }
    premiumUnlocked = true;
    setStoredPremiumAccess();
    const url = new URL(window.location.href);
    url.searchParams.set('premium', 'BUSANBLUE');
    window.history.replaceState({}, '', url.toString());
    closeQrScannerModal();
    configureMode();
    bindPremiumControlEvents();
    showPremiumUnlockedMessage();
    const recent = getRecentFreeCapture();
    if (recent?.imageBase64 && recent?.freeAnalysis) await startPremiumAnalysisFromExistingPhoto();
    else if (premiumQuality) {
      premiumQuality.hidden = false;
      premiumQuality.innerHTML = `<h3>${esc(t('premiumAccessConfirmed'))}</h3><p>${esc(t('premiumAccessNoPhoto'))}</p>`;
    }
  }
  ...
  function unlockPremiumAccess() {
    handlePremiumQrSuccess('BUSANBLUE');
  }
        ...
        try {
          ...
          if (raw) {
            if (isValidPremiumQr(raw)) {
              ...
              await handlePremiumQrSuccess(raw);
              return;
            }
            showQrResult(t('qrInvalid'));
          }
  ...
  function openQrScannerModal() {
    ...
    modal.querySelector('#vrmtCancelQrScanBtn')?.addEventListener('click', closeQrScannerModal);
