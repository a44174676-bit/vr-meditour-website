exports.handler = async function (event) {
  ...
  try {
    ...
    if (!openaiRes.ok) {
      console.error('AMIS OpenAI error status:', openaiRes.status);
      return response(openaiRes.status, { ok: false, error: 'AI 루트 생성 중 오류가 발생했습니다.' });
    }

    const outputText = payload.output_text || payload.output?.[0]?.content?.find((c) => c.type === 'output_text')?.text;
    if (!outputText) return response(502, { ok: false, error: 'AI 응답을 해석하지 못했습니다.' });

    let result;
    try { result = JSON.parse(outputText); } catch (_) { return response(502, { ok: false, error: 'AI 응답 형식이 올바르지 않습니다.' }); }
    result.ok = true;
    result.language = language;
    return response(200, result);
  } catch (error) {
    console.error('AMIS function error:', error?.name || 'unknown');
